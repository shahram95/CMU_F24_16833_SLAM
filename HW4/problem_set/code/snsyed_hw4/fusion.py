...
class Map:
    ...
    def merge(self, indices, points, normals, colors, R, t):
        ...
        self.points[indices] = (weights * self.points[indices] + points_world) / (weights + 1)
        self.normals[indices] = (weights * self.normals[indices] + normals_world) / (weights + 1)
    ...
    def filter_pass2(self, points, normals, input_points, input_normals,
        ...
        '''
        # Calculate point-wise distances using numpy's norm function
        distances = np.linalg.norm(points - input_points, axis=1)
        
        # Calculate angles between normals using dot product and arccos
        # Normalize the dot product to handle numerical precision issues
        norm_products = np.sum(normals * input_normals, axis=1)
        normal_magnitudes = np.linalg.norm(normals, axis=1) * np.linalg.norm(input_normals, axis=1)
        cos_angles = np.clip(norm_products / normal_magnitudes, -1.0, 1.0)  # Ensure numerical stability
        angles = np.abs(np.arccos(cos_angles))
        
        # Combine distance and angle masks
        return (distances < dist_diff) & (angles < angle_diff)
